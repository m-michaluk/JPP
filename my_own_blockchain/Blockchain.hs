...
mineBlock miner parent txs = Block (BlockHeader parent coinbase txroot nonce) txs where
  txroot = treeHash (buildTree (coinbase:txs))
  coinbase = coinbaseTx miner
  nonce = findNonce (minBound::Hash) where
    findNonce n = if validNonce (testHdr n) then n else findNonce (succ n)
    testHdr = BlockHeader parent coinbase txroot
...
mineTransactions miner parent txs = (block, receipts) where
    ...
    receipts = map buildReceipt txs where
      buildReceipt t = let (Just proof) = (buildProof t transTree) in TxReceipt h proof -- buildproof dla elementów z txs zwróci zawsze Just    
      h = hash block
      c = coinbaseTx miner
      transTree = buildTree (c:txs)
